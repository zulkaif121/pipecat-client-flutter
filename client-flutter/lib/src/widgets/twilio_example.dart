...
/// A simple example widget that demonstrates the Pipecat Flutter client
/// Similar to the test-client from naturalflow-website
class TwilioExample extends StatefulWidget {
  const TwilioExample({super.key});

  @override
  State<TwilioExample> createState() => _TwilioExampleState();
}
...
class _TwilioExampleState extends State<TwilioExample> {
  ...
  Widget build(BuildContext context) {
    return ChangeNotifierProvider.value(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          title: const Text(
            'Pipecat Flutter Example',
            style: TextStyle(color: Color(0xFFE0E6F1)),
          ),
  ...
  Widget _buildStatusIndicator(PipecatProvider provider) {
    Color color;
    if (provider.connected) {
      color = Colors.green;
    } else if (provider.connecting) {
      color = Colors.orange;
    } else {
      color = Colors.red;
    }
  ...
  String _getStatusText(PipecatProvider provider) {
    if (provider.connected) {
      return 'Connected';
    } else if (provider.connecting) {
      return 'Connecting...';
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildInstruction('5. Use the microphone button to mute/unmute yourself'),
          _buildInstruction('6. Tap the hang-up button to disconnect'),
        ],
      ),
    );
  }
